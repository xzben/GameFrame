...
-- 当前触摸是否是超出obj的范围的
local function is_overflow_touch(touch, obj)
...
function TouchHelper:add_touch_listener( obj, callback, filter_overflow, swallow)
	...
	local function onTouchBegan(touch, event)
		if filter_overflow and is_overflow_touch(touch, obj) then
			return false
		end
...
function TouchHelper:add_mutile_touch_listener( obj, callback )
	local func_begin  = callback[1]
	local func_end    = callback[2]
	local func_move   = callback[3]
	local func_cancel = callback[4]
	...
	local function onTouchBegan(touches, event)
		if func_begin then
			return func_begin(touches, event)
		end
		-- CCTOUCHBEGAN event must return true
		-- 如果return false 后面的触摸事件都不会发生
		return true
